# Table of contents

## [AI](ai)

- [Claude Code Local](ai/claude-code-local.md)

## [Ansible](ansible)

- [README](ansible/README.md)
- [Inventory](ansible/docs/inventory.md)
- [Playbooks](ansible/docs/playbooks.md)
- [GitLab Runner](ansible/playbooks/gitlab-runner.yml)
- [Install Docker](ansible/playbooks/install_docker.yml)

## [Databases](database)

- [Cassandra](database/cassandra.md)
- [Microsoft SQL](database/mssql.md)
- [MongoDB](database/mongodb.md)
- [MySQL](database/mysql.md)
- [PostgreSQL](database/custom-postgresql.conf)
- [SQLite](database/sqlite.md)

## [Docker](docker)

- [README](docker/README.md)
- [Brief Tutorial](docker/brief-tutorial.md)
- [Cheatsheet](docker/docker-cheatsheet.md)
- [Docker with Windows 10 Home](docker/docker-with-windows-home.md)
- [How Docker Works](docker/how-docker-works.md)
- [Docker Compose](docker/docs/docker-compose.md)

## [Go](go)

- [Installation](go/installation.md)

## [Linux](linux)

- [Cheatsheet](linux/cheatsheet.md)
- [Create Connection to MSSQL](linux/create_connection_to_mssql.md)
- [Fake Shell for FTP User](linux/fake_shell_for_ftp_user.md)
- [Network](linux/network.md)
- [Oh My Zsh](linux/oh-my-zsh.md)
- [PHP](linux/php.md)
- [Shell Functions](linux/shell-functions.md)
- [SSH](linux/ssh.md)

## [PHP](php)

- [PHP 7.1](php/php-7.1.md)
- [WordPress Learning Notes](php/wordpress-learning-notes.md)

## [Python](python)

- [Command Cheatsheet](python/command-cheatsheet.md)
- [Check TLS with Python 2.7.9](python/check_tls_2.7.9.py)
- [Check TLS with Python 3](python/check_tls_3.py)
- [Delete Compiled Python](python/delete-compiled-python.sh)
- [Django](python/django.md)
- [Format Function](python/format_function.md)
- [PyCharm](python/pycharm.md)
- [Relay Service](python/relay.py)
- [Upload to PyPI](python/upload-to-pypi.md)
- [Locust.io](python/locustio/00-intro.md)

## [Vagrant](vagrant)

- [Cheatsheet](vagrant/cheatsheet.md)
- [Self-hosted Vagrant Boxes with Versioning](vagrant/self-hosted-vagrant-boxes-with-versioning.md)
- [Vagrant with PyCharm](vagrant/vagrant-with-pycharm.md)
- [Vagrant with Docker](vagrant/vagrant-docker/Vagrantfile)
- [Vagrant with Kafka](vagrant/vagrant-kafka/Vagrantfile)
- [Vagrant Ubuntu 16.04](vagrant/vagrant-ubuntu_16.04/Vagrantfile)

## [Web](web)

- [Prevent iFrame Embedded](web/prevent-iframe-embedded.md)
- [Apache](web/web-server/apache.md)

## [Windows](windows)

- [Delete Duplicate Ebooks](windows/del_duplicate_ebooks.md)
- [VirtualBox](windows/virtualbox.md)
- [WSL Install](windows/wsl_install.sh)
- [WSL 2](windows/wsl2.md)

## [Others](others)

- [AMQP](others/amqp.md)
- [Atlassian](others/atlassian.md)
- [Compact JSON](others/compact-json.md)
- [Git](others/git.md)
- [ISO Message 8583](others/isomessage8583.md)
- [Nginx](others/nginx.md)
- [Postman](others/postman.md)
- [Sign and Verify by Pair Keys](others/sign-verify-by-pair-keys.md)
- [Sphinx](others/sphinx.md)
